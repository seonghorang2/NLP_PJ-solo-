...
- 분석 포함 리뷰 수(`included_review_count`)가 `100` 미만이면 분석/리포트 생성을 중단합니다.
- 결과를 `processed`에 저장합니다.

### Stage 2. 규칙 기반 분석 집계
- `included_review_count >= 100`인 경우에만 카테고리/테마/최근 변화 신호를 집계합니다.
...
## 4-1. 단계별 상세(현재 구현 기준)
아래는 실제 코드 기준으로 “각 단계에서 정확히 무엇을 하는지”를 풀어쓴 버전입니다.

### 1) 리뷰 수집(Ingestion)
- Steam 리뷰 API를 페이지 단위로 순회합니다.
- 기본 언어는 `koreana`, 정렬 필터는 `recent`를 사용합니다.
- 페이지 간 중복 리뷰 ID를 제거해 최종 raw 리스트를 만듭니다.
- 동시에 Steam appdetails에서 게임 메타데이터(장르/가격모델/출시상태)를 수집합니다.

### 2) 규칙 기반 전처리(Deterministic Preprocess)
- Markup cleaning: HTML/BBCode/script/style 제거
- 텍스트 정규화: 공백/반복문자 정리, 오타/표현 치환
- 한국어 비율(`hangul_ratio`) 계산
- 제외 판정:
  - 저품질 리뷰
  - 욕설-only 리뷰
  - 비한국어(한글 비율 낮음)
- ambiguity flags 생성(언어 혼합, 감정 혼합, 카테고리 모호성 등)
- 포함 리뷰에 한해 카테고리 태깅, 미분류 포함은 `unclassified_included` 표시
- `rule_decision`, `rule_confidence`, `final_decision_source` 흔적 저장

### 3) 규칙 기반 분석 집계(Analysis)
- 포함 리뷰 수가 `100` 이상일 때만 집계를 수행합니다.
- `100` 미만이면 이 단계는 실행하지 않고 리포트를 비노출 처리합니다.
- 계산 항목:
  - mention_count
  - negative_ratio
  - experienced_player_share(플레이타임 기반)
  - 최근 추세(up/down/flat/limited)
  - 대표 테마(top themes)
- 리뷰 수 규모에 따라 sample size tier를 부여합니다.

### 4) 리포트 재료 정제(Selective LLM Material Refiner)
- 분석 포함 리뷰 중 “리포트 쓰임새가 높은 후보”만 선택합니다.
- 기본 상한: `max_llm_reviews=50`
- 후보 우선순위: 태그/테마 유무, 플레이타임, 작성자 리뷰수, 길이, 최신성
...
1. report_plan 생성
2. section별 report_display 생성(headline/recommendation/strength/risk/recent_state)
3. evidence_sections 생성(강점/리스크 분리)
- 고합의(mention_count 높은) 의견을 우선 반영합니다.

### 6) 구매 근거(Evidence) 정합성 보정
- 근거 후보를 `strict -> relaxed -> guaranteed_fill` 3단계로 선별합니다.
- stance(긍/부정)와 theme 일치 여부를 하드 필터로 먼저 걸러냅니다.
- `title + why_it_matters` 기반 유사도 랭킹으로 후보를 재정렬합니다.
- 옵션으로 Evidence Judge LLM이 블록별 최종 후보를 재판정합니다.

### 7) 최종 후처리(Final Polish)
- 무료/유료에 따라 추천 문구를 가격모델에 맞게 치환합니다.
- 한국어 교정(rule + optional LLM proofread) 적용
- 내부 분류형 금지 라벨을 사용자 경험형 문장으로 치환합니다.

### 8) 저장 및 조회
- 산출물 저장:
  - raw / processed / analysis / metadata / report
- 사용자 조회 경로는 저장된 `report`만 읽는 read-only 방식입니다.
- 사용자 요청으로 Steam API 재호출/재분석/LLM 실행은 하지 않습니다.

## 4-2. 단계별 실제 데이터 예시 1세트 (raw -> processed -> analysis -> report)
아래 예시는 실제 저장 파일(`1245620(ELDEN RING).json`)에서 핵심 필드만 축약한 샘플입니다.
...
```json
{
  "review_id": "223345891",
  "voted_up": true,
  "playtime_at_review_hours": 8.42,
  "review_text": "와.. 진짜 맨땅 헤딩인데 뭐가 뭔지 아직도 모르겠다 ㅋㅋ ..."
}
```

### B) processed (전처리/규칙 판정 후)
```json
{
  "review_id": "223345891",
  "included_in_analysis": true,
  "rule_decision": "include",
  "rule_confidence": 0.7217,
  "category_tags": [],
  "canonical_theme": null,
  "ambiguity_flags": ["unclassified_included"],
  "normalized_text": "와.. 진짜 맨땅 헤딩인데 뭐가 뭔지 아직도 모르겠다 ㅋㅋ ..."
}
```
...
{
  "report_display": {
    "headline": "세계관과 연출 덕분에 플레이를 계속하게 되는 몰입 경험 장점은 분명하지만 ...",
    "buy_recommendation": "buy_on_sale",
    "buy_timing_summary": "가격 메리트가 있는 시점이 더 낫습니다."
  },
  "evidence_sections": {
    "strengths_block_count": 3,
    "risks_block_count": 1
  }
}
```
...
이 예시를 한 줄로 요약하면:
- raw에서 시작한 1개 리뷰가 processed에서 규칙 판정/플래그를 거치고,
- analysis에서 다수 리뷰 집계 신호로 합쳐진 뒤,
- report에서 구매 판단용 문장/근거 블록으로 재구성됩니다.
...
배지 표기 원칙(오해 방지):
- `현재 구현됨`: 현재 코드/데모에서 실제 동작하는 단계
- `향후 계획`: 실서비스 목표로만 정의된 단계(현재는 자동 실행 미구현)
...
┌──────────────────────────────────────────────────────────────┐
│ [통합 플로우 타임라인: 12단계]                               │
│ 1 수집 → 2 전처리+표본게이트(100) → 3 분석집계 → 4 LLM 재료정제 → │
│ 5 리포트생성 → 6 품질게이트 → 7 스냅샷저장 →                  │
│ 8 사용자 선택 → 9 리포트 반환 → 10 신규리뷰증가 →             │
│ 11 재분석필요판정 → 12 수동 재생성/스냅샷교체                 │
└──────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────┐
│ [선택 단계 상세 패널]                                          │
│ - 단계 설명 / 입력 / 출력 / 핵심 지표 / 핵심 규칙 / 상태 배지 │
└──────────────────────────────────────────────────────────────┘
```
...
flowchart LR
    ...
    A2 --> A2B{included_review_count >= 100?}
    A2B -- 아니오 --> A2C[리포트 비노출]
    A2B -- 예 --> A3[3. 규칙 기반 분석 집계]
    ...
    A8 --> A9[9. 리포트 반환]
    A9 --> A10[10. 신규 리뷰 증가]
    A10 --> A11[11. 재분석 필요 판정<br/>신규 리뷰 >= n]
    A11 --> A12[12. 수동 재생성/스냅샷 교체]
```

### 8-3. 단계별 mock JSON 스키마(입력/출력/지표)
- 실제 mock 데이터 파일:
...
### 11-2. 통합 플로우(오프라인 -> 온라인 -> 업데이트 루프)
1. 게임 전체 리뷰 수집 시작(최초 1회)
2. 규칙기반 전처리 + 표본 게이트(`included_review_count >= 100`)
3. 규칙 기반 분석 집계
4. LLM 활용 리포트 재료 정제
5. 리포트 생성
6. 품질 게이트
...
9. 리포트 반환
10. 신규 리뷰 추가
11. 자동 재분석 필요 판정(`신규 리뷰 >= 10`)
12. 수동 재수집/재분석/리포트 재생성 및 최신 스냅샷 교체
